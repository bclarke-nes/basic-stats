...
### Introduction
```{r eval=F, echo=F}
# Data from [NHS opendata](https://www.opendata.nhs.scot/dataset/df10dbd4-81b3-4bfa-83ac-b14a5ec62296/resource/a5d4de3f-e340-455f-b4e4-e26321d09207/download/12.2_birthweight.csv)
...
birthweight %>%
  ...
  ggplot() +
  geom_col(aes(x=Birthweight, y=Livebirths))
```

Imagine that we are measuring some health or care value in an individual This could be, just for example:
...
So far, we haven't done much to try and describe the frequency distributions that we've looked at. Instead, we've really just looked at their graphical representation by plotting them on a bar chart, so that we can see where a single value (Susan's, in this case) sits.

However, much of the power of statistics comes from more precise ways of describing distributions. In this section, we'll cover some simple tools for doing this kind of **descriptive statistics**. We'll use some data like this:
...
From here, we'll also start to include some of the code that we're using to do our statistical work at this point, so keep an eye out for that. We'll explain the statistical concept, and then introduce the code that you'll use, one after another. 

Luckily, the first of these descriptive statistics that we'll introduce will be very familiar: how many observations do we have here in total? The data that we're working on looks like this:
...
Okay, so we have `r nrow(descriptive_data)` observations in total. What's next?
...
```{r eval=params$r_only, echo=params$r_only}
# no built-in mode in R, but lots of good ways to calculate one.
descriptive_data %>%
...
* what's the problem with efd - cumbersome
* what's the answer - theoretical distributions
